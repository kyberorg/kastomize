...
.PHONY: help clean dep build binary
...
dep: ## Download the dependencies.
	go mod download && go get github.com/kyberorg/kastomize/cmd/kastomize

build: dep ## Build executable.
	mkdir -p ./bin
	CGO_ENABLED=0 GOOS=linux GOARCH=${GOARCH} go build ${LDFLAGS} -o bin/${BINARY} ./cmd/kastomize
